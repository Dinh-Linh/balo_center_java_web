...
public interface OrderDetailRepo extends JpaRepository<OrderDetail, Long> {
    ...
    @Query("SELECT new com.example.balo_center.domain.dto.dashboard.TopSellingProductDTO(od.product.productName, SUM(od.quantity)) "
            +
            "FROM OrderDetail od GROUP BY od.product.productName ORDER BY SUM(od.quantity) DESC")
